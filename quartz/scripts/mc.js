...
function run(
  ...
  try {
    ...
    let prefix = `docker run -m ${allocatedRAM}.1g -i -v ${absolutePath}/servers/${id}:/server -w /server -p ${port}:${port}/tcp -p ${port}:${port}/udp -p ${port + 66}:${port + 66}/tcp -p ${port + 33}:${port + 33}/udp --network host --user 1000:1000 --cpuset-cpus="${threadsString}" eclipse-temurin:${javaVer} java`;
