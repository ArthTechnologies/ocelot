  ...
  function tick() {
    ...
    progress.currentPacks.forEach((pack, i) => {
      ...
      slots.push({
        ...
        archiveDownload: mc.getArchiveDownloadProgress(slotIds[i]),
      });
    });
...
// Raw manifest for whichever pack a reserved check slot is currently
// installing - lets an admin watching the live check confirm the manifest
// actually looks like a real CurseForge/Modrinth modpack manifest. Exists
// because a pack can ship its own unrelated manifest.json inside its server
// pack (e.g. one generated by the third-party ServerPackCreator tool, whose
// `files` are plain path strings, not {projectID, fileID} mod references) -
// the install code only checks that a file by that name exists, so it's easy
// for a pack like that to get silently misrouted into the per-mod CurseForge
// download path and fail every "mod" at once. This is a debugging window
// into that, not something the install pipeline itself reads.
//
// Checked in the order the install writes them: curseforge.index.json is
// copied out of temp/ before the per-mod download loop even starts, so it's
// visible for the whole download phase; modrinth.index.json only lands at
// the very end, once every download has settled; temp/manifest.json is the
// pre-copy fallback for the brief window before that copy happens.
router.get("/modpack-checks/manifest/:id", (req, res) => {
  const modpackChecker = require("../scripts/modpackChecker.js");
  const id = parseInt(req.params.id, 10);
  if (!Number.isInteger(id) || !modpackChecker.checkServerIds().includes(id)) {
    return res.status(400).json({ error: "Not a reserved modpack-check slot." });
  }

  const candidates = [
    { file: `servers/${id}/curseforge.index.json`, source: "curseforge.index.json" },
    { file: `servers/${id}/modrinth.index.json`, source: "modrinth.index.json" },
    { file: `servers/${id}/temp/manifest.json`, source: "temp/manifest.json" },
  ];

  // Guard against a pathological manifest (or something that isn't a
  // manifest at all) freezing the modal on a multi-MB payload.
  ...
  for (const candidate of candidates) {
    ...
    try {
      ...
      try {
        return res.json({ available: true, source: candidate.source, manifest: JSON.parse(raw) });
      } catch (e) {
        // Present but not valid JSON - show it verbatim rather than erroring,
        // that's itself useful information about what actually got extracted.
        return res.json({ available: true, source: candidate.source, truncated: false, raw });
      }
    } catch (e) {
      continue;
    }
