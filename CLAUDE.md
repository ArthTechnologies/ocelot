...
#### Player Access (Whitelist / Blacklist)
`routes/server/access.js`, mounted at `/server/:id/access`, backs the Players panel's access controls. Every answer is derived from the files Minecraft itself owns — `server.properties`, `whitelist.json`, `banned-players.json`, `banned-ips.json`, `ops.json` — so a change made from the console, over FTP or by a plugin shows up in the panel without the panel having recorded it.
- The panel's two "modes" are just the two states of `white-list`, and the UI names them that way — **Whitelist Off** (`white-list=false`, everyone in except the banned) and **Whitelist On** (`white-list=true`, nobody in except the listed) — rather than inventing a "Blacklist Mode" that sounds like a feature you switch on. `GET /` reports that plus the whitelist, ban list, attempted joins, ops and Bedrock support; the rest are `POST /mode`, `POST|DELETE /whitelist`, `POST /kick`, `POST /ban`, `POST /unban`.

**A running server owns these files.** It holds the whitelist and ban list in memory and rewrites them whenever they change, so a file edit under a live server is ignored or clobbered on its next save. Every mutation therefore has two paths — a console command while the server is up, a direct file edit while it's down:
- **Whitelist add/remove goes through the file on both paths**, then `whitelist reload`. Not `whitelist add <name>`, because that makes the *server* do the name lookup and the server only knows how to ask Mojang — which fails outright for every Bedrock player. Resolving the entry panel-side and asking the server to re-read the file works identically for both platforms.
- **Ban prefers the command when running**, because `ban` both writes `banned-players.json` and disconnects the player with the reason on their screen; a file edit would leave them playing until they next logged out. Offline, the entry is built by hand in Minecraft's exact shape (`created` as `yyyy-MM-dd HH:mm:ss Z`, `expires: "forever"`) or the server discards the file on its next boot. An offline ban needs a uuid, so it's only offered for players already in `usercache.json`.
- **Kick has no file to fall back to** — it only exists as an instruction to a live process, so it 409s on a stopped server.
- **Turning the whitelist on kicks the non-whitelisted players explicitly.** `whitelist on` alone only turns new arrivals away at the door; whether it also removes the players already in the world is decided by `enforce-whitelist`, and that is read out of `server.properties` **once at boot**. The file write therefore cannot reach a running server, so a mid-session switch would otherwise leave everyone connected until they happened to log out — which reads as the switch not having worked. `enforce-whitelist` is still written for the next boot.
- Names and reasons are stripped of newlines before reaching `writeTerminal` — a `\n` in a reason field would otherwise run whatever followed it as a second console command. Not a privilege boundary (the same account has full console access via `/server/:id/terminal`), but a player named on a ban screen shouldn't be able to smuggle one in.

**Bedrock players are first-class here.** Geyser lets them connect; Floodgate gives them a Java-side identity, and both are detected separately because the distinction decides what can be offered — no Floodgate means no reliable way to turn a gamertag into a uuid, so the Bedrock add option is disabled rather than failing later.
- A Bedrock player is identified by *either* signal, since a whitelist entry can carry one without the other: a name starting with Floodgate's `username-prefix` (read back out of `plugins/floodgate/config.yml`, default `.` — it's configurable, so it can't be hardcoded), or a uuid starting `00000000-0000-0000-`. That uuid is synthesized from the player's Xbox XUID rather than issued by Mojang.
- The prefix is stripped for display (`displayName`) but kept on `name`, which is what every command and file entry has to use.
- Adding one resolves the gamertag through Geyser's global API (`api.geysermc.org/v2/utils/uuid/bedrock_or_java/:name?prefix=`), which applies the prefix and the space-to-underscore rewrite itself. `platform` is passed explicitly by the UI rather than guessed, because a gamertag and a Java username can be the same string.

**Attempted joins are read out of the server's own log, not inferred.** `usercache.json` only holds people who actually got in, so it cannot tell "tried to join and was refused" apart from "joined before the whitelist went up" — the only honest source is the line the server writes when it turns someone away (`Disconnecting <name> (/<ip>): You are not white-listed on this server!`; vanilla writes `white-listed`, Paper `whitelisted`, and older versions print the whole `GameProfile` — all three are matched). Only the tail of `logs/latest.log` is read, memoised on size+mtime so a 15s poll doesn't re-parse a quarter-megabyte for nothing.
- **The time is passed through verbatim and only the date is reconstructed.** That timestamp is on the *Minecraft server's* clock, which under Docker is usually UTC while the panel host is not; rebuilding an absolute time from it silently shifts every entry by the offset between the two. The parser therefore returns `lastAttemptTime` as printed plus `lastAttemptDaysAgo` (counted back from the log's mtime, incremented whenever the clock appears to jump forward while walking newest-first, which can only mean an earlier day), and the frontend renders the date from the anchor and the time as-is.
- **Filtering the list has to match on name, not uuid.** Modern servers log a bare name with no uuid, so a uuid-only check against the whitelist would keep showing someone under "attempted joins" immediately after you let them in.
- `POST /whitelist` takes an optional `uuid`: when the panel already knows the identity (a usercache entry, or a profile the log recorded) it skips the lookup entirely, which also means letting someone in still works when Mojang is unreachable. A malformed uuid falls through to the normal lookup rather than being written.

Frontend: `PlayerList.svelte` (mode badge doubles as the button), with `AccessModeModal.svelte` explaining and switching the two modes and `PlayerActionModal.svelte` confirming kick/ban/unban/unwhitelist. Each mode gets one collapsed-by-default section — **Banned** while the whitelist is off, **Attempted joins** while it's on — since both are things you check when someone complains, not things you watch. A player who isn't whitelisted is kept out of the main roster while the whitelist is on and appears in that section instead, with an **Allow** button; rows label themselves apart by what's actually known (`N attempts · last Aug 18, 16:00` from the log, versus `Has played here, but isn't whitelisted` from usercache). The list also shows whitelisted players who have **never joined** — otherwise adding someone appears to do nothing until they first log in.
...
**Server packs are what customers actually install.** When a CurseForge modpack version has a paired server pack — the file's `serverPackFileId`, falling back to the older `alternateFileId` only when `serverPackFileId` is absent (a file can carry both, pointing at two different server packs; `serverPackFileId` is the more complete one) — `ModpackVersion.svelte` silently swaps the install's `downloadUrl`/`versionId` over to it and shows a "Server Pack" badge — the client-pack file the version row nominally represents is never downloaded. Server packs bundle every jar directly and carry **no `manifest.json`**, so `downloadModpack()` takes its no-manifest branch: nothing is fetched per-mod, no `curseforge.index.json` is written (so the in-place update flow can't detect the installed version), and mods sit under their original filenames rather than `cf_{id}_CFMod.jar`. This also means the manual-mod hold never engages — mods whose authors disabled third-party API downloads arrive pre-bundled (packs like DawnCraft ship them with permission), which is why a pack can look "17 mods blocked" through the client manifest yet install completely for customers.
...
Both match on the file name lowercased with `-`/`_` stripped, so a bare token like `radium` also catches `Radium-mc1.18.2-0.12.2.jar`. Conflict rules only consider live `.jar` files, so a jar already renamed to `.jar.disabled` is neither a trigger nor a target and re-running the check is a no-op. Both run only on the modpack install path — mods uploaded manually afterwards are not filtered. They run on **every** branch of that path, including the no-manifest (server pack) branch, where the jars arrive pre-bundled from the zip rather than downloaded per-mod; a server pack ships client-only mods just as a client pack does. Both are safe no-ops when `mods/` is absent, which is what a failed download or unzip leaves behind on that same branch.

**The CurseForge manifest branch can't rely on `deleteClientSideMods()` at all.** Every mod on that path is saved as `cf_{projectID}_CFMod.jar` (see `downloadModpack()` in `mc.js`), so the post-download filename match never sees the mod's real name — for years this meant client-only mods installed via the standard manifest flow were never actually stripped. `assets/clientsidemodids.txt` (`name<TAB>curseforgeProjectId`, generated by resolving each `clientsidemods.txt` entry to its CurseForge project) fixes this at the source: `loadClientSideModProjectIds()` loads it once per `downloadModpack()` call, and the per-mod download loop checks `file.projectID` against it *before* calling `fetchCurseForgeDownloadUrl()` — a match is skipped outright, so the jar is never fetched at all rather than downloaded and then deleted. Skips are recorded into the same `modFilterStats[id].removedClientSide` list `deleteClientSideMods()` populates (now reset once at the top of `downloadModpack()` rather than inside `deleteClientSideMods()` itself, so the two don't clobber each other), so the checker's `expected`/`removedClientSide` accounting is unaffected. `clientsidemodids.txt` is necessarily incomplete — plenty of `clientsidemods.txt` entries have no confident CurseForge match — so `deleteClientSideMods()` still runs afterward as the filename-based catch-all for the modrinth and server-pack branches, where real filenames are preserved.
...
`scripts/modpackChecker.js` boots the most-downloaded modpacks `SLOT_COUNT` (currently **3**) at a time and records whether each reaches an online state. The slots drain a shared work queue (each pulls the next pack the moment its current one settles — a slow pack doesn't stall the other slots), and `progress.currentPacks` is **slot-indexed** with `null` for a drained slot, each entry carrying its own `phase`; the stream endpoint and both admin components skip the nulls. Runs **weekly** (Sunday 04:15) as the `checkModpacks` system task, or on demand via the `checkModpacks` console command. Results land in `logs/modpackChecks.json`.
- Coverage: Forge (CurseForge) only, on every version in `FORGE_GAME_VERSIONS` — `1.18.2, 1.12.2, 1.20.1, 1.16.5` — ranked and fetched **per version**. `topForgeModpacks` pages CurseForge's ranked search (`TOP_N`=12 candidates per page) until `TARGET_CHECKED_PER_VERSION` (**15**) packs actually resolve to something checkable, not just discovered — a single fixed-size page used to leave a version with only however many of its top candidates survived `resolveForgePack` (no file for that version, distribution disabled, a Forge tag that turned out to be a false positive), often well under the target. Paging stops early if CurseForge runs out of results, or after `MAX_CANDIDATES_PER_VERSION` (**60**) candidates looked at, so a version genuinely thin on Forge packs can't page forever. Unresolvable candidates are still kept in the results (as `skipped`, with a reason) for visibility — they just don't count toward the target. That's up to ~60 packs checked per run across all four versions, each of which can take two attempts, which is why the schedule is weekly rather than 12-hourly. `run.js` rewrites the cron of an existing `checkModpacks` task on boot, so panels created before the switch move to weekly on their next restart. Fabric is no longer in the batch run, but `resolveFabricPack` is kept so the admin recheck button (`checkOneModpack`) still works on old `mr` rows in the log.
- Ranking is by download count: CurseForge `sortField=6` (TotalDownloads, **not** 2/Popularity, which blends recency and relevance).
- **Forge packs are checked as their server pack when one exists**, mirroring the customer flow (see the server-pack note under Mods & Plugins): `resolveForgePack` walks the fetched file list (newest first) and installs the newest file whose `serverPackFileId` (or `alternateFileId` when no `serverPackFileId` is set) resolves to a server pack — even when the very latest file has none — marking the pack (and its result's `mods` object) `serverPack: true`. If no server pack resolves, it falls back to the newest client file rather than skipping the check. Because a server pack has no manifest, the checker can't wait on the index-file rewrite — `waitForModpackInstall` instead polls `mc().isModpackDownloadSettled(id)`, a flag on the per-server download record that every `downloadModpack()` exit path sets. Finishes name the record they belong to, so a download abandoned by an earlier timed-out pack can't settle a newer install in the shared slot. Server-pack results report `expected`/`manifest` as 0 with the real `installed` count (an empty `mods/` after extraction is an outright failure), and the admin modal labels them accordingly. `installed` counts live `.jar` files *after* the client-side/conflict filters have run on the bundle, so `removedClientSide`/`disabledByConflict` are real numbers on server-pack rows too and the log line and modal report them.
...
- Uses **`SLOT_COUNT` reserved server slots** — ids `modpackCheckServerId` (config.txt, default `50000`) through `+ SLOT_COUNT - 1`, so raising the constant claims more consecutive ids — each wiped between packs. A slot is refused at startup if it falls inside `[idOffset, idOffset + maxServers)` — the range `/server/reserve` hands to customers — or if `10000 + id + 66` isn't a valid port; the run refuses if *any* slot fails that check, naming the offenders.
- Each slot is deliberately **smaller than a customer server**: `CHECK_RAM_GB` (5) via `ramOverrideGB` and `CHECK_CPU_CORES` (1) via `cpuCoresOverride`, both read by `run()` in `mc.js`. Three slots at one core each is roughly the footprint two four-core slots had, so the higher concurrency doesn't cost the customer servers sharing the box more CPU than before. One core does mean slower boots, which trades against `START_TIMEOUT_MS`.
- `cpuCoresOverride` is a general `server.json` field, not a checker-only one: `run()` pins every server to four threads from a rotating pool unless it asks for fewer, clamped to the thread count the machine actually reports.
