...
<script lang="ts" context="module">
    ...
    function condenseTimestamp(timeStr: string): string {
      return timeStr.replace(/\[(\d{1,2}):(\d{2}):(\d{2})( [A-Za-z]+)?\]/g, (_, hours, minutes, _seconds, level) => {
...
send(input);
  ...
  function renderTerminalLines() {
    ...
    const shouldCondenseLines = get(lineCondensation);
    ...
    // expanded stay in seenLines but out of collapsedLines, so they keep their
    // state across re-renders and pref toggles.
    rows.forEach((row) => {
      if (!seenLines.has(row.lineNum)) {
        seenLines.add(row.lineNum);
        collapsedLines.add(row.lineNum);
      }
    });
    ...
    rows.forEach((row) => {
      ...
      const isCollapsed = shouldCondenseLines && collapsedLines.has(lineNum);
      ...
      <div class="relative" bind:this={prefsRef}>
        ...
        {#if showPrefs}
          <div class="terminal-prefs-dropdown">
            ...
            <label class="flex items-center gap-2 py-1 cursor-pointer">
              <input type="checkbox" bind:checked={$lineCondensation} class="checkbox checkbox-xs" />
              <span class="text-xs text-gray-300">Line Condensation</span>
            </label>
            <label class="flex items-center gap-2 py-1 cursor-pointer">
