<script lang="ts">
    ...
    if (browser) {
        fetch(apiurl+"server/"+localStorage.getItem("serverID")+"/storageInfo", {
        method: "GET",
        headers: {
...
<div id="ratioVisualizer" class="bg-base-200 border-base-300 border-2 rounded-lg px-4 flex items-center w-44 font-semibold text-sm uppercase text-white gap-1.5"><HardDrive/> Using {storageRatio}</div>